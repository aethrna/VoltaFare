...
class DBHelper(context: Context) : SQLiteOpenHelper(context, "UserDB", null, 3) {
        ...
        fun getUserProfile(email: String): Map<String, String?> {
            ...
            try {
                if (cursor.moveToFirst()) {
                    ...
                    profile[COLUMN_DISPLAY_NAME] =
                        cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_DISPLAY_NAME))
                    profile[COLUMN_DISPLAY_TITLE] =
                        cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_DISPLAY_TITLE))
                    profile[COLUMN_PROFILE_IMAGE_URI] =
                        cursor.getString(cursor.getColumnIndexOrThrow(COLUMN_PROFILE_IMAGE_URI))
                    profile[COLUMN_XP] =
                        cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_XP)).toString()
                    profile[COLUMN_LEVEL] =
                        cursor.getInt(cursor.getColumnIndexOrThrow(COLUMN_LEVEL)).toString()
                }
            } finally {
                cursor.close()
                db.close()
